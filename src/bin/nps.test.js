...
test('nps: run without args', t =>
  tester(babel, require.resolve('./nps'))
    .then(({code, stdout, stderr}) => {
      t.is(code, 0, 'should exit with code 0')
      t.regex(stdout, /Usage: nps \[options]/, 'should show help by default')
...
test('nps: run with config', t => Promise.resolve()
  .then(() => writeFooBarConfig(t))
  .then(() => tester(babel, require.resolve('./nps'), '--config', tmpConfig(t.title))) // eslint-disable-line ava/use-t-well
  .then(({stdout}) => {
